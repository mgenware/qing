...
func (da *TableTypeDiscussion) deleteCmtChild1(queryable mingru.Queryable, cmtID uint64) (DiscussionTableDeleteCmtChild1Result, error) {
	var result DiscussionTableDeleteCmtChild1Result
	err := queryable.QueryRow("SELECT `discussion_cmt`.`host_id` AS `host_id`, `join_1`.`reply_count` AS `cmt_reply_count` FROM `discussion_cmt` AS `discussion_cmt` INNER JOIN `cmt` AS `join_1` ON `join_1`.`id` = `discussion_cmt`.`cmt_id` WHERE `discussion_cmt`.`cmt_id` = ?", cmtID).Scan(&result.HostID, &result.CmtReplyCount)
	if err != nil {
		return result, err
	}
	return result, nil
...
// DeleteCmt ...
func (da *TableTypeDiscussion) DeleteCmt(db *sql.DB, cmtID uint64, id uint64, userID uint64) error {
...
func (da *TableTypeDiscussion) deleteReplyChild1(queryable mingru.Queryable, id uint64) (DiscussionTableDeleteReplyChild1Result, error) {
	...
	err := queryable.QueryRow("SELECT `reply`.`parent_id` AS `parent_id`, `join_2`.`host_id` AS `parent_id_host_id` FROM `reply` AS `reply` INNER JOIN `cmt` AS `join_1` ON `join_1`.`id` = `reply`.`parent_id` INNER JOIN `discussion_cmt` AS `join_2` ON `join_2`.`cmt_id` = `join_1`.`id` WHERE `reply`.`id` = ?", id).Scan(&result.ParentID, &result.ParentIdHostID)
	if err != nil {
		return result, err
	}
	return result, nil
