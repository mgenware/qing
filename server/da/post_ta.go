...
// EditItem ...
func (da *TableTypePost) EditItem(queryable mingru.Queryable, id uint64, userID uint64, title string, content string, modifiedAt time.Time, sanitizedStub int) error {
	result, err := queryable.Exec("UPDATE `post` SET `title` = ?, `content` = ?, `modified_at` = ? WHERE (`id` = ? AND `user_id` = ?)", title, content, modifiedAt, id, userID)
	return mingru.CheckOneRowAffectedWithError(result, err)
}
...
func (da *TableTypePost) insertItemChild1(queryable mingru.Queryable, title string, content string, userID uint64, createdAt time.Time, modifiedAt time.Time) (uint64, error) {
	result, err := queryable.Exec("INSERT INTO `post` (`title`, `content`, `user_id`, `created_at`, `modified_at`, `cmt_count`, `likes`) VALUES (?, ?, ?, ?, ?, 0, 0)", title, content, userID, createdAt, modifiedAt)
	return mingru.GetLastInsertIDUint64WithError(result, err)
}
...
// InsertItem ...
func (da *TableTypePost) InsertItem(db *sql.DB, title string, content string, userID uint64, createdAt time.Time, modifiedAt time.Time, sanitizedStub int, captStub int) (uint64, error) {
	var insertedIDExported uint64
	txErr := mingru.Transact(db, func(tx *sql.Tx) error {
		var err error
		insertedID, err := da.insertItemChild1(tx, title, content, userID, createdAt, modifiedAt)
		if err != nil {
			return err
		}
...
// SelectCmtsWithLike ...
func (da *TableTypePost) SelectCmtsWithLike(queryable mingru.Queryable, hostID uint64, viewerUserID uint64, page int, pageSize int) ([]CmtData, bool, error) {
	if page <= 0 {
		err := fmt.Errorf("Invalid page %v", page)
		return nil, false, err
	}
	...
	offset := (page - 1) * pageSize
	max := pageSize
	rows, err := queryable.Query("SELECT `post_cmt`.`cmt_id` AS `id`, `join_1`.`content` AS `content`, `join_1`.`created_at` AS `created_at`, `join_1`.`modified_at` AS `modified_at`, `join_1`.`reply_count` AS `reply_count`, `join_1`.`likes` AS `likes`, `join_1`.`user_id` AS `user_id`, `join_2`.`name` AS `user_name`, `join_2`.`icon_name` AS `user_icon_name`, `join_3`.`user_id` AS `hasLiked` FROM `post_cmt` AS `post_cmt` INNER JOIN `cmt` AS `join_1` ON `join_1`.`id` = `post_cmt`.`cmt_id` INNER JOIN `user` AS `join_2` ON `join_2`.`id` = `join_1`.`user_id` LEFT JOIN `cmt_like` AS `join_3` ON `join_3`.`host_id` = `post_cmt`.`cmt_id` WHERE (`post_cmt`.`host_id` = ? AND (`join_3`.`user_id` IS NOT NULL AND `join_3`.`user_id` = ?)) ORDER BY `created_at` DESC LIMIT ? OFFSET ?", hostID, viewerUserID, limit, offset)
	if err != nil {
		return nil, false, err
	}
	result := make([]CmtData, 0, limit)
