...
func (da *TableTypeForum) SelectDiscussions(queryable mingru.Queryable, forumID *uint64, page int, pageSize int) ([]UserThreadInterface, bool, error) {
	...
	max := pageSize
	rows, err := queryable.Query("SELECT 3 AS `threadType`, `discussion`.`id` AS `id`, `discussion`.`user_id` AS `user_id`, `join_1`.`name` AS `user_name`, `join_1`.`icon_name` AS `user_icon_name`, `discussion`.`created_at` AS `RawCreatedAt`, `discussion`.`modified_at` AS `RawModifiedAt`, `discussion`.`title` AS `title`, `discussion`.`reply_count` AS `value1`, 0 AS `value2`, 0 AS `value3`, `discussion`.`last_replied_at` AS `last_replied_at` FROM `discussion` AS `discussion` INNER JOIN `user` AS `join_1` ON `join_1`.`id` = `discussion`.`user_id` WHERE `discussion`.`forum_id` = ? ORDER BY `last_replied_at` LIMIT ? OFFSET ?", forumID, limit, offset)
	if err != nil {
		return nil, false, err
	}
...
func (da *TableTypeForum) SelectQuestions(queryable mingru.Queryable, forumID *uint64, page int, pageSize int) ([]UserThreadInterface, bool, error) {
	...
	offset := (page - 1) * pageSize
	max := pageSize
	rows, err := queryable.Query("SELECT 2 AS `threadType`, `question`.`id` AS `id`, `question`.`user_id` AS `user_id`, `join_1`.`name` AS `user_name`, `join_1`.`icon_name` AS `user_icon_name`, `question`.`created_at` AS `RawCreatedAt`, `question`.`modified_at` AS `RawModifiedAt`, `question`.`title` AS `title`, `question`.`likes` AS `value1`, `question`.`reply_count` AS `value2`, 0 AS `value3`, `question`.`last_replied_at` AS `last_replied_at` FROM `question` AS `question` INNER JOIN `user` AS `join_1` ON `join_1`.`id` = `question`.`user_id` WHERE `question`.`forum_id` = ? ORDER BY `last_replied_at` LIMIT ? OFFSET ?", forumID, limit, offset)
	if err != nil {
		return nil, false, err
	}
	result := make([]UserThreadInterface, 0, limit)
...
func (da *TableTypeForum) SelectThreads(queryable mingru.Queryable, forumID *uint64, page int, pageSize int) ([]UserThreadInterface, bool, error) {
	...
	offset := (page - 1) * pageSize
	max := pageSize
	rows, err := queryable.Query("(SELECT 3 AS `threadType`, `discussion`.`id` AS `id`, `discussion`.`user_id` AS `user_id`, `join_1`.`name` AS `user_name`, `join_1`.`icon_name` AS `user_icon_name`, `discussion`.`created_at` AS `RawCreatedAt`, `discussion`.`modified_at` AS `RawModifiedAt`, `discussion`.`title` AS `title`, `discussion`.`reply_count` AS `value1`, 0 AS `value2`, 0 AS `value3`, `discussion`.`last_replied_at` AS `last_replied_at` FROM `discussion` AS `discussion` INNER JOIN `user` AS `join_1` ON `join_1`.`id` = `discussion`.`user_id` WHERE `discussion`.`forum_id` = ? LIMIT ? OFFSET ?) UNION (SELECT 2 AS `threadType`, `question`.`id` AS `id`, `question`.`user_id` AS `user_id`, `join_1`.`name` AS `user_name`, `join_1`.`icon_name` AS `user_icon_name`, `question`.`created_at` AS `RawCreatedAt`, `question`.`modified_at` AS `RawModifiedAt`, `question`.`title` AS `title`, `question`.`likes` AS `value1`, `question`.`reply_count` AS `value2`, 0 AS `value3`, `question`.`last_replied_at` AS `last_replied_at` FROM `question` AS `question` INNER JOIN `user` AS `join_1` ON `join_1`.`id` = `question`.`user_id` WHERE `question`.`forum_id` = ? LIMIT ? OFFSET ?) ORDER BY `last_replied_at` DESC LIMIT ? OFFSET ?", forumID, limit, offset, forumID, limit, offset, limit, offset)
	if err != nil {
		return nil, false, err
	}
	result := make([]UserThreadInterface, 0, limit)
