...
func (mrTable *ContentBaseCmtStaticAGType) SelectRootCmts(mrQueryable mingru.Queryable, contentBaseCmtTableParam mingru.Table, hostID uint64, page int, pageSize int, orderBy1 ContentBaseCmtStaticAGSelectRootCmtsOrderBy1, orderBy1Desc bool) ([]DBCmt, bool, error) {
	...
	max := pageSize
	rows, err := mrQueryable.Query("SELECT `content_base_cmt_table_param`.`cmt_id` AS `id`, `join_1`.`content`, `join_1`.`created_at`, `join_1`.`modified_at`, `join_1`.`cmt_count`, `join_1`.`likes`, `join_1`.`del_flag`, `join_1`.`user_id`, `join_1`.`parent_id`, `join_2`.`name`, `join_2`.`icon_name` FROM "+string(contentBaseCmtTableParam)+" AS `content_base_cmt_table_param` INNER JOIN `cmt` AS `join_1` ON `join_1`.`id` = `content_base_cmt_table_param`.`cmt_id` LEFT JOIN `user` AS `join_2` ON `join_2`.`id` = `join_1`.`user_id` WHERE `content_base_cmt_table_param`.`host_id` = ? ORDER BY "+orderBy1SQL+", `join_1`.`id` LIMIT ? OFFSET ?", hostID, limit, offset)
	if err != nil {
		return nil, false, err
	}
...
func (mrTable *ContentBaseCmtStaticAGType) SelectRootCmtsUserMode(mrQueryable mingru.Queryable, contentBaseCmtTableParam mingru.Table, viewerUserID uint64, hostID uint64, page int, pageSize int, orderBy1 ContentBaseCmtStaticAGSelectRootCmtsUserModeOrderBy1, orderBy1Desc bool) ([]DBCmt, bool, error) {
	...
	offset := (page - 1) * pageSize
	max := pageSize
	rows, err := mrQueryable.Query("SELECT `content_base_cmt_table_param`.`cmt_id` AS `id`, `join_1`.`content`, `join_1`.`created_at`, `join_1`.`modified_at`, `join_1`.`cmt_count`, `join_1`.`likes`, `join_1`.`del_flag`, `join_1`.`user_id`, `join_1`.`parent_id`, `join_2`.`name`, `join_2`.`icon_name`, `join_3`.`user_id` AS `is_liked` FROM "+string(contentBaseCmtTableParam)+" AS `content_base_cmt_table_param` INNER JOIN `cmt` AS `join_1` ON `join_1`.`id` = `content_base_cmt_table_param`.`cmt_id` LEFT JOIN `user` AS `join_2` ON `join_2`.`id` = `join_1`.`user_id` LEFT JOIN `cmt_like` AS `join_3` ON `join_3`.`host_id` = `content_base_cmt_table_param`.`cmt_id` AND `join_3`.`user_id` = ? WHERE `content_base_cmt_table_param`.`host_id` = ? ORDER BY "+orderBy1SQL+", `join_1`.`id` LIMIT ? OFFSET ?", viewerUserID, hostID, limit, offset)
	if err != nil {
		return nil, false, err
	}
	result := make([]DBCmt, 0, limit)
...
func (mrTable *ContentBaseCmtStaticAGType) SelectRootCmtsUserModeFilterMode(mrQueryable mingru.Queryable, contentBaseCmtTableParam mingru.Table, viewerUserID uint64, hostID uint64, excluded []uint64, page int, pageSize int, orderBy1 ContentBaseCmtStaticAGSelectRootCmtsUserModeFilterModeOrderBy1, orderBy1Desc bool) ([]DBCmt, bool, error) {
	...
	rows, err := mrQueryable.Query("SELECT `content_base_cmt_table_param`.`cmt_id` AS `id`, `join_1`.`content`, `join_1`.`created_at`, `join_1`.`modified_at`, `join_1`.`cmt_count`, `join_1`.`likes`, `join_1`.`del_flag`, `join_1`.`user_id`, `join_1`.`parent_id`, `join_2`.`name`, `join_2`.`icon_name`, `join_3`.`user_id` AS `is_liked` FROM "+string(contentBaseCmtTableParam)+" AS `content_base_cmt_table_param` INNER JOIN `cmt` AS `join_1` ON `join_1`.`id` = `content_base_cmt_table_param`.`cmt_id` LEFT JOIN `user` AS `join_2` ON `join_2`.`id` = `join_1`.`user_id` LEFT JOIN `cmt_like` AS `join_3` ON `join_3`.`host_id` = `content_base_cmt_table_param`.`cmt_id` AND `join_3`.`user_id` = ? WHERE (`content_base_cmt_table_param`.`host_id` = ? AND `join_1`.`id` NOT IN ("+mingru.InputPlaceholders(len(excluded))+")) ORDER BY "+orderBy1SQL+", `join_1`.`id` LIMIT ? OFFSET ?", queryParams...)
	if err != nil {
		return nil, false, err
	}
	result := make([]DBCmt, 0, limit)
