...
func (da *TableTypeQuestion) deleteCmtChild1(queryable mingru.Queryable, cmtID uint64) (QuestionTableDeleteCmtChild1Result, error) {
	var result QuestionTableDeleteCmtChild1Result
	err := queryable.QueryRow("SELECT `question_cmt`.`host_id` AS `host_id`, `join_1`.`reply_count` AS `cmt_reply_count` FROM `question_cmt` AS `question_cmt` INNER JOIN `cmt` AS `join_1` ON `join_1`.`id` = `question_cmt`.`cmt_id` WHERE `question_cmt`.`cmt_id` = ?", cmtID).Scan(&result.HostID, &result.CmtReplyCount)
	if err != nil {
		return result, err
	}
	return result, nil
...
// DeleteCmt ...
func (da *TableTypeQuestion) DeleteCmt(db *sql.DB, cmtID uint64, id uint64, userID uint64) error {
...
func (da *TableTypeQuestion) deleteReplyChild1(queryable mingru.Queryable, id uint64) (QuestionTableDeleteReplyChild1Result, error) {
	...
	err := queryable.QueryRow("SELECT `reply`.`parent_id` AS `parent_id`, `join_2`.`host_id` AS `parent_id_host_id` FROM `reply` AS `reply` INNER JOIN `cmt` AS `join_1` ON `join_1`.`id` = `reply`.`parent_id` INNER JOIN `question_cmt` AS `join_2` ON `join_2`.`cmt_id` = `join_1`.`id` WHERE `reply`.`id` = ?", id).Scan(&result.ParentID, &result.ParentIdHostID)
	if err != nil {
		return result, err
	}
	return result, nil
