...
func (mrTable *TableTypeContentBaseCmtUtil) insertCmtChild3(mrQueryable mingru.Queryable, cmtHostTable mingru.Table, hostID uint64) error {
	result, err := mrQueryable.Exec("UPDATE "+cmtHostTable.MingruSQLName()+" SET `cmt_count` = `cmt_count` + 2 WHERE `id` = ?", hostID)
	return mingru.CheckOneRowAffectedWithError(result, err)
}

// InsertCmt ...
func (mrTable *TableTypeContentBaseCmtUtil) InsertCmt(db *sql.DB, contentHTML string, userID uint64, cmtHostTable mingru.Table, hostID uint64, sanitizedStub int, captStub int) (uint64, error) {
...
func (mrTable *TableTypeContentBaseCmtUtil) insertReplyChild3(mrQueryable mingru.Queryable, cmtHostTable mingru.Table, hostID uint64) error {
	result, err := mrQueryable.Exec("UPDATE "+cmtHostTable.MingruSQLName()+" SET `cmt_count` = `cmt_count` + 2 WHERE `id` = ?", hostID)
	return mingru.CheckOneRowAffectedWithError(result, err)
}

// InsertReply ...
func (mrTable *TableTypeContentBaseCmtUtil) InsertReply(db *sql.DB, parentID uint64, contentHTML string, userID uint64, cmtHostTable mingru.Table, hostID uint64, sanitizedStub int, captStub int) (uint64, error) {
	...
	txErr := mingru.Transact(db, func(tx *sql.Tx) error {
		...
		err = mrTable.insertReplyChild3(tx, cmtHostTable, hostID)
		if err != nil {
			return err
		}
