...
func NewDiscussionPageModel(p *da.DiscussionTableSelectItemByIDResult, msgListHTML string, pageBarHTML string) DiscussionPageModel {
	...
	d.UserHTML = rcom.GetUserItemViewHTML(p.UserID, p.UserName, p.UserIconName, p.UserStatus, eid, defs.Shared.EntityDiscussion, d.CreatedAt, d.ModifiedAt)
...
func NewDiscussionMsgModel(p *da.DiscussionMsgTableSelectItemsByDiscussionResult) DiscussionMsgModel {
	...
	d.EID = eid
	d.UserHTML = rcom.GetUserItemViewHTML(p.UserID, p.UserName, p.UserIconName, p.UserStatus, eid, defs.Shared.EntityDiscussionMsg, d.CreatedAt, d.ModifiedAt)
	return d
}
