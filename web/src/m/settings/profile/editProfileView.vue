  ...
  <div>
    ...
    <article class="message m-t-md is-light">
      ...
      <div class="message-body">
        ...
        <button class="button is-success" @click="handleSaveProfileClick">{{$ls.save}}</button>
      </div>
    </article>
    ...
    <article class="message m-t-md is-light">
      ...
      <div class="message-body">
        <Editor ref="bioEditor" :value="profileData.Bio || ''"/>
        <button type="button" class="button is-success" @click="handleSaveBioClick">{{$ls.save}}</button>
      </div>
    </article>
  </div>
</template>
...
export default class EditProfileApp extends Vue {
  @Prop() profileData!: EditProfileData;
  bioEditor: Editor | null = null;

  mounted() {
    this.bioEditor = this.$refs.bioEditor as Editor;
  }
  ...
  private async handleSaveBioClick() {
    if (!this.bioEditor) {
      return;
    }
    const content = (this.bioEditor as any).getValue();
    const loader = new SetBioLoader(content);
    const res = await app.runActionAsync(loader, ls.saving);
    if (res.isSuccess) {
      this.profileData.Bio = content;
      await app.alert.successToast(ls.saved);
    }
  }
}
