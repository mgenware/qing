...
*/\n\n`;
const entries = ['app', 'infra', 'frozen'];
const outputDir = './g';
const header =
  copyrightString +
  ` /******************************************************************************************
...
async function buildJSONFileAsync(src, name) {
  ...
  const jsContent = sc.js.convert(jsonObj);
  const dtsContent = sc.js.convert(jsonObj, { dts: true });

  await Promise.all([
    mfs.writeFileAsync(`${outputDir}/${name}.js`, jsContent),
    mfs.writeFileAsync(`${outputDir}/${name}.d.ts`, dtsContent),
    mfs.writeFileAsync(`${`../../server/a/def/${goNamespace}/${name}.go`}`, header + goResult),
  ]);
}

async function buildConstantsAsync() {
  return Promise.all(entries.map((f) => buildJSONFileAsync(`./src/${f}.json`, f)));
}
